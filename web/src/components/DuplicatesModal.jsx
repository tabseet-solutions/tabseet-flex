...
export default function DuplicatesModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-40" onClick={onClose}>
      <div
        className="bg-base-900 rounded-lg w-full max-w-3xl max-h-[85vh] flex flex-col p-4"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto space-y-4 -mx-1 px-1">
          ...
          {!loading &&
            groups?.map((group) => (
              <div key={group.key} className="bg-base-800 rounded-lg p-3">
                <p className="text-sm font-medium mb-2 truncate" title={group.copies[0].name}>
                  {group.copies[0].name}
                </p>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {group.copies.map((c) => (
                    <div key={c.id} className="bg-base-700 rounded overflow-hidden">
                      <img
                        src={thumbUrl(c.id)}
                        alt=""
                        loading="lazy"
                        className="w-full aspect-video object-cover bg-base-700"
                      />
                      <div className="p-2 text-xs space-y-1">
                        <p className="truncate font-medium" title={c.rootDisplay}>
                          {c.rootDisplay}
                        </p>
                        <p className="text-gray-400">{formatBytes(c.size)}</p>
                        <button
                          onClick={() => requestKeep(group, c)}
                          disabled={busyKey === group.key}
                          className="w-full mt-1 px-2 py-1 rounded bg-accent-500 hover:bg-accent-400 disabled:opacity-50"
                        >
        ...
        >
          <div className="bg-base-900 rounded-lg w-full max-w-md p-4" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-400 mb-3">
              Keeping <strong className="text-gray-200">{confirming.keep.rootDisplay}</strong>. This will
              permanently delete:
            </p>
            <ul className="text-sm mb-4 space-y-1">
              {confirming.deleteList.map((c) => (
                <li key={c.id} className="flex justify-between gap-2">
                  ...
                  <span className="text-gray-400 flex-shrink-0">{formatBytes(c.size)}</span>
                </li>
              ))}
            </ul>
            <p className="text-xs text-red-400 mb-4">This cannot be undone.</p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setConfirming(null)} className="px-3 py-1.5 rounded text-sm hover:bg-base-700">
                Cancel
              </button>
              <button onClick={confirmKeep} className="px-3 py-1.5 rounded text-sm bg-red-600 hover:bg-red-500">
